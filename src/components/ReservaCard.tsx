...
export interface Reserva {
  ...
  precio: number;
  tipoViaje: 'ida' | 'ida-vuelta' | 'multiple';
}
...
const ReservaCard: React.FC<ReservaCardProps> = ({ reserva, onClick }) => {
  ...
  const getEstadoIcon = (estado: Reserva['estado']) => {
    switch (estado) {
      case 'pendiente':
        ...
        <div className="space-y-3 mb-4">
          ...
          <div className="flex items-center text-gray-600">
            ...
            <span className="text-sm">
              {reserva.fechaSalida} - {reserva.fechaRegreso}
            </span>
          </div>
          
          <div className="flex items-center text-gray-600">
            <Users className="w-4 h-4 mr-2 text-blue-500" />
            <span className="text-sm">{reserva.pasajeros} pasajero(s)</span>
          </div>
        </div>
