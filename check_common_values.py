...
try:
    with get_db_connection() as conn:
        ...
        print("--- Common cd_sucursal in VueloIDA ---")
        cursor.execute("SELECT TOP 5 cd_sucursal, COUNT(*) FROM dbo.VueloIDA GROUP BY cd_sucursal ORDER BY COUNT(*) DESC")
        for row in cursor.fetchall():
            print(row)
            
        print("\n--- Common iden_gds in VueloIDA ---")
        cursor.execute("SELECT TOP 5 iden_gds, COUNT(*) FROM dbo.VueloIDA GROUP BY iden_gds ORDER BY COUNT(*) DESC")
        for row in cursor.fetchall():
            print(row)
